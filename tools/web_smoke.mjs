...
function staticChecks() {
  ...
  check(["view-anarchess", "view-solo", "view-anarcheckers"].every((id) =>
    html.includes('id="' + id + '"')) && !html.includes('id="an-mode"'),
  "the three land games have separate browser pages");
  const refs = new Set();
...
`);
  ...
  check(levels.length >= 15, "bridge.levels", levels.length + " levels");
  check(levels[0].elo < levels[levels.length - 1].elo
    && levels[0].depth < levels[levels.length - 1].depth,
  "levels are graded by Elo and depth");
  const pawnCap = bridge("analyse",
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
    "Pawn", 1200, "standard", null);
  check(pawnCap.depth <= levels[0].depth,
    "browser analysis keeps the selected level depth cap",
    "d" + pawnCap.depth + " cap d" + levels[0].depth);
  ...
  // The browser exposes four opponent profiles and three independent rulesets.
  const landLevels = bridge("anarchess_levels");
  check(landLevels.length === 4 && landLevels.at(-1).name === "Warlord",
    "land-game opponent profiles reach Warlord");

  // anarchess: a full turn for every player, driven by the bot
  const game = bridge("anarchess_new", 2, null, 12345);
  const warlordTurn = bridge("anarchess_bot", JSON.stringify(game), 4, 17);
  const warlordFirst = warlordTurn.actions && warlordTurn.actions[0];
  const warlordApplied = warlordFirst && bridge("anarchess_apply", JSON.stringify(game),
    JSON.stringify(warlordFirst));
  check(Array.isArray(warlordTurn.actions) && warlordTurn.actions.length > 0
    && warlordApplied && warlordApplied.ok,
    "the browser bridge actually accepts a Warlord turn");
