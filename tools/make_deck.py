  ...
  <div class="body">
    ...
    <div class="cols">
      <div>
        <h3>A turn</h3>
        <ol>
          <li>The <b>die names a colour</b>: that is the tile you must lay,
            and the tribe of the <em>other</em> colour plays it.</li>
          <li><b>Then one pawn action</b>, if you want it: settle a pawn from
            your reserve onto the tile you just laid, step one square, or
            attack across a diagonal.</li>
          <li>A tile touching only one other must be of the <b>opposite</b>
            colour. Attacks need no support — a lone pawn can take one.</li>
        </ol>
      </div>
      <div>
        <h3>Scoring</h3>
        <p>An <b>area</b> is a maximal group of edge-connected tiles of one
          colour. Areas of two or more tiles score for whichever tribe holds
          most of them: <b>2</b> points a tile, <b>3</b> when the area matches
          its owner's colour or is held by a lone pawn, but only <b>1</b> in
          the largest area, which the rules tax. Single tiles are worth
          nothing, and every pawn left in the reserve costs <b>6</b>.</p>
        <p><small>SOLO is one player against a target of 192; Anarcheckers
          steps and captures on the diagonals, with forced jumps.</small></p>
      </div>
    </div>
  </div>
</section>
