...
- **The board is not frozen between moves.** Each piece shifts its weight
  while it waits: the knight paws the ground, the bishop sways, the queen looks
  about, the king fidgets, and the rook does very nearly nothing at all. The
  offsets are a hair's breadth, they are suspended the moment a piece starts to
  move, and no two pieces breathe in time with each other. Preferences can turn
  it off.
